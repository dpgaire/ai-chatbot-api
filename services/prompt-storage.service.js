...
class PromptStorageService {
  constructor() {
    ...
    this.collectionName = process.env.PROMPT_STORAGE_COLLECTION_NAME || 'prompt-storage';
  ...
  async addPrompt(promptData, userId) {
    await this.ensureCollection();

    const embedding = await this.geminiManager.generateEmbedding(promptData.title);
    const id = promptData.id || generateId();

    const point = {
      id: id,
      vector: embedding,
      payload: { ...promptData, userId },
    };
  ...
  async getPromptById(id) {
    ...
    if (response.length === 0) {
      throw new Error(`Prompt with id ${pointId} not found.`);
    }
    return { id: response[0].id, ...response[0].payload };
  }

  async updatePrompt(id, promptData, userId, role) {
    await this.ensureCollection();
    const pointId = normalizeId(id);
  ...
  async deletePrompt(id, userId, role) {
    await this.ensureCollection();
    try {
      const pointId = normalizeId(id);
...
module.exports = new PromptStorageService();
