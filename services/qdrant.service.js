...
class QdrantManager {
  ...
  async ensureCollection(collectionName, vectorSize = 768) {
    try {
      ...
      const collectionExists = await this.client.getCollection(collectionName).catch(() => null);
      if (collectionExists) {
  ...
  async getUserQueries() {
    ...
    const scrollResult = await this.client.scroll(this.userQueriesCollectionName, {
      with_payload: true,
      with_vectors: false,
