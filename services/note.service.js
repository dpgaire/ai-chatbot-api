...
class NoteService {
  constructor() {
    ...
    this.collectionName = process.env.NOTE_COLLECTION_NAME || 'notes';
  ...
  async addNote(noteData, userId) {
    ...
    const embedding = await this.geminiManager.generateEmbedding(noteData.content);
    const id = noteData.id || generateId();

    const point = {
      id: id,
      vector: embedding,
      payload: { ...noteData, userId },
    };
  ...
  async getNoteById(id) {
    ...
    if (response.length === 0) {
      throw new Error(`Note with id ${pointId} not found.`);
    }
    return { id: response[0].id, ...response[0].payload };
  }

  async updateNote(id, noteData, userId, role) {
    await this.ensureCollection();
    const pointId = normalizeId(id);
